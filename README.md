...
下表は**元データ(A22-2016)の全フィールド**です。`build-data.sh` はファイル軽量化のため
地図で参照するフィールドのみを残すので、**生成物 `web/data/gosetsu.geojson` に含まれるのは
「含む」列が ○ のフィールド(A22_002/003/006/007/008/009)だけ**です。
A22_001/004/005 は出力に含まれません。

| フィールド | 内容 | gosetsu.geojson に含む |
|---|---|:---:|
| A22_001 | 豪雪地帯ID | −(dissolveにより非保持) |
| A22_002 | 行政区域コード | ○ |
| A22_003 | 都道府県名 | ○ |
| A22_004 | 支庁名(北海道のみ) | −(地図で未使用のため削除) |
| A22_005 | 郡名 | −(地図で未使用のため削除) |
| A22_006 | 市区町村名 | ○ |
| A22_007 | 市区町村名(現在) | ○ |
| A22_008 | 旧市町村に関する備考 | ○ |
